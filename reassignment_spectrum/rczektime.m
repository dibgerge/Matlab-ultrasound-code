function [Tr] = rczektime(varargin)
%   rczektime - compute the relaxation time of a signal
%               using the formula of Rihaczek, computed from  
%               the second partial derivative of the short
%               time phase spectrum.
%
%				This is pretty much untested.
%---------------------------------------------------------------------
%   Tr = rczektime(A,NFFT,Fs,window,olap)
%
%   Tr =     Rihaczek relaxation time
%   A =      signal
...
% -- compute second derivative of phase w.r.t time --
deriv = dphasedt2( A, NFFT, Fs, W, olap );
...
Tr = 1.0 ./ max( eps, sqrt( abs( deriv ) ) );
              
% --------------- argument checking function ---------------

function [msg,x,nfft,Fs,win,nolap] = check_args(args)
%   Extract and check up to five arguments to rczektime
%   from an array.
%
...
if length(args) > 3 & ~isempty(args{4})
    ...
    if length(nfft) == 1
        ...
        msg = 'rczektime: You must specify a window function.';
    end
end
if length(win) == 1
    win = hanning(win); 
...
if (length(nfft)==1) & (nfft<length(win)),
    msg = 'rczektime: Window length cannot be greater than the FFT length.';
end
if (nolap >= length(win)),
    msg = 'rczektime: Overlap must be less than the window length.';
end
if (length(nfft)==1) & (nfft ~= abs(round(nfft)))
    msg = 'rczektime: FFT length must be a positive integer.';
end
if (nolap ~= abs(round(nolap))),
    msg = 'rczektime: Overlap must be a positive integer.';
end
if min(size(x))~=1,
    msg = 'rczektime: Input signal must be a vector (either row or column).';
end
