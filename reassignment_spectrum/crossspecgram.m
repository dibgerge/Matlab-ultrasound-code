function [Bo,Fo,To] = crossspecgram(varargin)
%   crossspecgram - compute a reassigned spectrogram
%   of a signal using Nelson's cross-spectral 
%   surfaces to approximate the partial derivatives.
% ---------------------------------------------------
%   [B,F,T] = crossspecgram(A,NFFT,Fs,window,olap)
%   [B,F] = crossspecgram(A,NFFT,Fs,window,olap)
%   B = crossspecgram(A,NFFT,Fs,window,olap)
%   crossspecgram(A,NFFT,Fs,window,olap)
%
...
%
%   See also crossspectrum, raspecgram, raspectrum.
...
% pass them to fft.
%
% it looks like maybe the offset vector is one term
% shorter than it should be? Should go to length(A)-length(W)?
% -kel 21 Sep 2006
hop = length(W)-olap;
...
C = STFT .* conj(STFTdel);
L = STFT .* conj(STFTfreqdel);

% compute channelized instaneous frequency, a matrix of 
% remapped frequency vectors, one vector for each time step 
% for which ffts were computed
%
...
% one (row) vector for each discrete frequency step determined 
% by fft length
%
% The mod 2pi wraps negative differences up to positive
% differences (the absolute value of the difference can
...
%   Extract and check up to five arguments to crossspecgram
%   from an array.
%
...
if length(args) > 3 & ~isempty(args{4})
    ...
    if length(nfft) == 1
        ...
        msg = 'crossspecgram: You must specify a window function.';
    end
end
if length(win) == 1
    win = hanning(win); 
...
if (length(nfft)==1) & (nfft<length(win)),
    msg = 'crossspecgram: Window length cannot be greater than the FFT length.';
end
if (nolap >= length(win)),
    msg = 'crossspecgram: Overlap must be less than the window length.';
end
if (length(nfft)==1) & (nfft ~= abs(round(nfft)))
    msg = 'crossspecgram: FFT length must be a positive integer.';
end
if (nolap ~= abs(round(nolap))),
    msg = 'crossspecgram: Overlap must be a positive integer.';
end
if min(size(x))~=1,
    msg = 'crossspecgram: Input signal must be a vector (either row or column).';
end
