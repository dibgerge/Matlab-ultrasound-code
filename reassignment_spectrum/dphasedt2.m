function [ deriv ] = dphasedt2(varargin)
%   dphasedt2 - compute the second partial derivative of 
%               spectral phase w.r.t. time
% ---------------------------------------------------------
%   deriv = dphasedt2(A,NFFT,Fs,window,olap)
%
%   deriv =  second derivative of phase w.r.t. time
%   A =      signal
...
Wdd = timederivwindow(Wd,Fs);

% -- compute auxiliary spectra --
Xd = specgram(A,NFFT,Fs,Wd,olap);
Xdd = specgram(A,NFFT,Fs,Wdd,olap);
...
term1 = imag( Xdd .* conj(X) ) ./ magsqrd;
term2 = imag( (Xd .* conj(X)) .^2 ) ./ ( magsqrd.^2 );
deriv = term1 - term2;
              
% --------------- argument checking function ---------------

function [msg,x,nfft,Fs,win,nolap] = check_args(args)
%   Extract and check up to five arguments to dphasedt2
%   from an array.
%
...
if length(args) > 3 & ~isempty(args{4})
    ...
    if length(nfft) == 1
        ...
        msg = 'dphasedt2: You must specify a window function.';
    end
end
if length(win) == 1
    win = hanning(win); 
...
if (length(nfft)==1) & (nfft<length(win)),
    msg = 'dphasedt2: Window length cannot be greater than the FFT length.';
end
if (nolap >= length(win)),
    msg = 'dphasedt2: Overlap must be less than the window length.';
end
if (length(nfft)==1) & (nfft ~= abs(round(nfft)))
    msg = 'dphasedt2: FFT length must be a positive integer.';
end
if (nolap ~= abs(round(nolap))),
    msg = 'dphasedt2: Overlap must be a positive integer.';
end
if min(size(x))~=1,
    msg = 'dphasedt2: Input signal must be a vector (either row or column).';
end
