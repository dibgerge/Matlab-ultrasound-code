function [X,F,T] = findifspectrum(varargin)
%   findifspectrum - compute a reassigned spectrum
%   of a signal using finite differences of phase 
%   to approximate the partial derivatives.
% --------------------------------------------------------------
%   [X,F,T] = findifspectrum(A,NFFT,Fs,window)
%
...
%            for each point in X
%
%   See also findifspecgram, raspecgram, raspectrum.
...
% -- make sure that A is not too long, and invoke findifspecgram --
Nx = min(length(A), length(W)+1);
A = A(1:Nx);
[X,F,T] = findifspecgram(A,NFFT,Fs,W);
...
%   Extract and check up to four arguments to findifspectrum
%   from an array.
%
...
if length(args) > 3 & ~isempty(args{4})
    ...
    if length(nfft) == 1
        ...
        msg = 'findifspectrum: You must specify a window function.';
    end
end
if length(win) == 1
    win = hanning(win); 
...
if (length(nfft)==1) & (nfft<length(win)),
    msg = 'findifspectrum: Window length cannot be greater than the FFT length.';
end
if (length(nfft)==1) & (nfft ~= abs(round(nfft)))
    msg = 'findifspectrum: FFT length must be a positive integer.';
end
if min(size(x))~=1,
    msg = 'findifspectrum: Input signal must be a vector (either row or column).';
end
