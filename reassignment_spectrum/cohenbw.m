function [Bw,F,T] = cohenbw(varargin)
%   cohenbw - compute bandwidth of a signal using the
...
%               by Cohen.
%---------------------------------------------------------------------
%   [Bw,F,T] = cohenbw(A,NFFT,Fs,window,olap)
%
%   Bw =     Cohen bandwidths (unknown units)
...
[rows,cols] = size(X);
Bw = zeros(rows,cols);
Bw(nonzero) = abs( real(Xdt(nonzero).*conj(X(nonzero)))./magsqrd(nonzero) );
              

% --------------- argument checking function ---------------

function [msg,x,nfft,Fs,win,nolap] = check_args(args)
%   Extract and check up to five arguments to cohenbw
%   from an array.
%
...
if length(args) > 3 & ~isempty(args{4})
    ...
    if length(nfft) == 1
        ...
        msg = 'cohenbw: You must specify a window function.';
    end
end
if length(win) == 1
    win = hanning(win); 
...
if (length(nfft)==1) & (nfft<length(win)),
    msg = 'cohenbw: Window length cannot be greater than the FFT length.';
end
if (nolap >= length(win)),
    msg = 'cohenbw: Overlap must be less than the window length.';
end
if (length(nfft)==1) & (nfft ~= abs(round(nfft)))
    msg = 'cohenbw: FFT length must be a positive integer.';
end
if (nolap ~= abs(round(nolap))),
    msg = 'cohenbw: Overlap must be a positive integer.';
end
if min(size(x))~=1,
    msg = 'cohenbw: Input signal must be a vector (either row or column).';
end
