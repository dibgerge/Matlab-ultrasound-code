function [deriv] = hainsworthfreq(varargin)
%   hainsworthfreq -
%               compute the partial derivative of the short
%               time amplitude spectrum with respect to frequency,
%               as proposed by Stephen Hainsworth and Malcolm
%               Macleod. This is said to be equivalent, after
%               scaling, to frequency reassignment. See 
%               "Time-frequency reassignment: a review and 
%               analysis", technical report, Cambridge University 
%               Engineering Dept, 2003.
%
%				This is pretty much untested.
%---------------------------------------------------------------------
%   deriv = hainsworthfreq(x,NFFT,Fs,window,olap)
%
%   deriv =  Hainsworth and Macleod's frequency derivative of 
%            amplitude spectrum
%   x =      signal
...
deriv(nonzero) = imag(Xt(nonzero).*conj(X(nonzero)))./magsqrd(nonzero); 

              
% --------------- argument checking function ---------------

function [msg,x,nfft,Fs,win,nolap] = check_args(args)
%   Extract and check up to five arguments to hainsworthfreq
%   from an array.
%
...
if length(args) > 3 & ~isempty(args{4})
    ...
    if length(nfft) == 1
        ...
        msg = 'hainsworthfreq: You must specify a window function.';
    end
end
if length(win) == 1
    win = hanning(win); 
...
if (length(nfft)==1) & (nfft<length(win)),
    msg = 'hainsworthfreq: Window length cannot be greater than the FFT length.';
end
if (nolap >= length(win)),
    msg = 'hainsworthfreq: Overlap must be less than the window length.';
end
if (length(nfft)==1) & (nfft ~= abs(round(nfft)))
    msg = 'hainsworthfreq: FFT length must be a positive integer.';
end
if (nolap ~= abs(round(nolap))),
    msg = 'hainsworthfreq: Overlap must be a positive integer.';
end
if min(size(x))~=1,
    msg = 'hainsworthfreq: Input signal must be a vector (either row or column).';
end
