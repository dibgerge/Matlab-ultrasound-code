function [X,F,T] = raspectrum(varargin)
%   raspectrum - compute a reassigned spectrum 
%   of a signal using the method of Auger and 
%   Flandrin to compute the partial derivatives 
%   from STFTs.
% ---------------------------------------------------------------------
%   [X,F,T] = raspectrum(A,NFFT,Fs,window)
%
...
%            for each point in X
%
%   See also findifspectrum, crossspectrum, raspecgram.
...
% -- make sure that A is no longer than W, and invoke raspecgram --
Nx = min(length(A), length(W));
A = A(1:Nx);
[X,F,T] = raspecgram(A,NFFT,Fs,W);
...
%   Extract and check up to five arguments to raspectrum
%   from an array.
%
...
if length(args) > 3 & ~isempty(args{4})
    ...
    if length(nfft) == 1
        ...
        msg = 'raspectrum: You must specify a window function.';
    end
end
if length(win) == 1
    win = hanning(win); 
...
if (length(nfft)==1) & (nfft<length(win)),
    msg = 'raspectrum: Window length cannot be greater than the FFT length.';
end
if (length(nfft)==1) & (nfft ~= abs(round(nfft)))
    msg = 'raspectrum: FFT length must be a positive integer.';
end
if min(size(x))~=1,
    msg = 'raspectrum: Input signal must be a vector (either row or column).';
end
